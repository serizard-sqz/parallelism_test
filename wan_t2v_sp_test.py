#!/usr/bin/env python3
"""Wan 2.2 T2V sequence-parallel Diffusers benchmark script."""
...
def frames_for_export(frames):
    """WanPipeline returns np.stack'd video (B, F, H, W, C); imageio expects a list of (H, W, C) frames."""
...
def parse_args():
    ...
    parser.add_argument("--model-id", default="Wan-AI/Wan2.2-T2V-A14B-Diffusers", help="Target Wan 2.2 model id.")
    ...
    parser.add_argument("--lora-subfolder", default="Wan2.2-T2V-A14B-4steps-lora-rank64-Seko-V2.0", help="Subfolder inside --lora-repo containing the LoRA file.")
    ...
    parser.add_argument("--attention-backend", default="_native_cudnn", help="Optional Diffusers transformer attention backend, e.g. _native_cudnn.")
    ...
    parser.add_argument("--output-dir", default="outputs/t2v", help="Directory for generated videos and default latency log.")
    ...
    parser.add_argument("--prompt", default="A cinematic shot of a mountain lake at sunrise, detailed, high quality.", help="Text prompt for T2V generation.")
...
def main():
    ...
    pipe = WanPipeline.from_pretrained(args.model_id, vae=vae, **load_kwargs)
    pipe.to(device)
    ...
    for run_index in range(args.warmup_runs + args.benchmark_runs):
        ...
        pipe_kwargs = {"prompt": args.prompt, "negative_prompt": args.negative_prompt, "height": height, "width": width, "num_frames": args.num_frames, "num_inference_steps": args.num_inference_steps, "guidance_scale": args.guidance_scale, "generator": generator}
